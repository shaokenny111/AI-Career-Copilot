...
export default function UploadPage() {
  ...
  const [parsed, setParsed] = useState<ResumeParseOutput | null>(null);
  ...
  const uploaded = status === "done" && !!parsed;
  ...
  async function processText(text: string, name: string) {
    ...
    try {
      ...
      const master = buildMaster(p, c.resumeType);
      const store = loadStorage();
      saveStorage({ ...store, master });
