...
5. 「项目背景 / 项目概述 / 项目简介」这类位于 bullet 之前的描述性开头行，
   也属于该段经历的 content，必须【完整保留】，不得删除或压缩——这些行往往
   含实质事实（如具体地名、产品、业务场景），丢了会让后续改写缺事实、脑补。
   正确做法：把"背景行 + 各 bullet"一并放进 content。
...
- links：把页眉/抬头里出现的个人主页、作品集、GitHub、LinkedIn 等网址都收进来，
  每条 { "label": 名称, "url": 网址 }（如 Web/个人网站→label "个人主页"）；
  没有则省略 links 或给空数组。不要漏掉原文出现的链接。

【绝对禁止】
- 编造原文不存在的经历、公司、项目
- 把 content 改写、润色或精简（这一步只做搬运和归类）
- 删除「项目背景/概述」等描述性开头行（它们是 content 的一部分）
...
输入（简历纯文本）：
邵子康  shaokenny@example.com  138-0000-0000  上海  Web: https://zikang.space
AI 产品经理，金融科技背景

工作经历
海晟金融租赁 | 产品经理 | 2022.07 - 至今
负责租赁业务线数字化系统建设，主导需求梳理与跨部门协作，推动业务流程线上化。

项目经验
乡村振兴项目 | 项目组成员 | 2020.09 - 2021.09
项目背景：学院乡村振兴项目，承接增城邓山村项目，推动当地特色农产品乌榄的商业化与产业化，增加居民收入。
驻村调研：通过驻村、访谈调研邓山村乌榄产业，撰写尽调材料。
落地支持：对接加工厂与销售渠道，推动乌榄衍生产品落地。
  ...
  "segments": [
    {
      ...
      "time_range": { "start": "2022-07", "end": "present" },
      "is_current": true,
      "tags": ["产品经理", "跨部门协作"]
    },
    {
      "type": "project",
      "title": "乡村振兴项目 项目组成员",
      "content": "项目背景：学院乡村振兴项目，承接增城邓山村项目，推动当地特色农产品乌榄的商业化与产业化，增加居民收入。驻村调研：通过驻村、访谈调研邓山村乌榄产业，撰写尽调材料。落地支持：对接加工厂与销售渠道，推动乌榄衍生产品落地。",
      "time_range": { "start": "2020-09", "end": "2021-09" },
      "is_current": false,
      "tags": ["乡村振兴", "尽职调查"]
    },
    {
...
注意：上例中"项目背景：…"整行被完整放进了 project 的 content（不是被删掉），
且页眉的 Web 网址被收进了 basic_info.links。`;
...
function toContract(raw: z.infer<typeof rawSchema>): ResumeParseOutput {
  const basicInfo: BasicInfo = {
    ...
    ...(raw.basic_info.links?.length ? { links: raw.basic_info.links } : {}),
  };
